"""
Apply cascade delete migration to YouTube database
Removes CASCADE from video_notes and adds storage cleanup triggers
"""
...
# Initialize Supabase client for YouTube database
...
def apply_migration():
    ...
    print("=" * 60)
    print("YouTube Database - Cascade Delete Migration")
    print("=" * 60)
    
    try:
        # Read migration file
        migration_file = os.path.join(os.path.dirname(__file__), "..", "database_cascade_migration.sql")
        with open(migration_file, 'r') as f:
            migration_sql = f.read()
        
        print("\n[MIGRATION] Applying cascade delete changes...")
        print("\nChanges to be applied:")
        print("  1. Remove CASCADE from video_notes foreign key")
        print("  2. Add storage cleanup trigger for videos")
        print("  3. Add storage cleanup trigger for chunks")
        print()
        ...
        print("  ✅ subtitle_chunks: CASCADE (deleted when video deleted)")
        print("  ✅ video_notes: NO ACTION (preserved when video deleted)")
        print("  ✅ Storage: Handled by backend delete_video() function")
        print()
